...
class threshold:
    ...
    def merge(d, target_file, df_data, learn_file, work_dir, target_word=None):
        headerlist     = []
        datalist       = []
        ...
        filename = learn_dir + "/learn_" + str(name_target) + ".csv"
        df = pd.DataFrame(df)
        df.to_csv(filename)
        return filename

    #やっていることはマージだが閾値更新なのでupdateと再定義している
    def update(d, target_file, df_data, learn_file, work_dir, target_word=None):
        learn_f = threshold.merge(d, target_file, df_data, learn_file, work_dir, target_word)
        return learn_f

#閾値とデータをCSV化、グラフ化するためのクラス
class vtt:
    def vtt(n, d, learn_file, target_file, df_data, work_dir, target_word=None, metric=None, service=None, makecsv=None, makefig=None):
        df_learn = pd.read_csv(learn_file, index_col=0)
        ...
        dir_his = work_dir + "/history"
        dir_his_png = dir_his + "/png"
        dir_his_csv = dir_his + "/csv"
        basic.check_dir(dir_his)
        basic.check_dir(dir_his_png)
        basic.check_dir(dir_his_csv)
        if makecsv == None or makecsv == True:
            out_csv = df_new.to_csv(dir_his_csv +"/thshld_" + name + ".csv")
        if makefig == None or makefig == True:
            ...
            plt.savefig(dir_his_png + "/thrshld_" + name + ".png")
            plt.close('all')
